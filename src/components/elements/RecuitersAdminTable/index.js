...
const RecruitersAdminTable = () => {
  const [page, setPage] = useState(0);
  const [sort, setSort] = useState('-id');
  const { data, isLoading } = useQuery(['recruiters', { page, sort }], () => recruiterApi.get({ page, sort }));
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {isLoading ? (
              ...
              data?.data.items?.map((recruiter) => <RecruiterRow recruiter={recruiter} key={recruiter.id} />)
            )}
          </tbody>
        </table>
      </div>
...
export default RecruitersAdminTable;
