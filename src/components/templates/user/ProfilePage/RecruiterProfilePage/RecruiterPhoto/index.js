...
const RecruiterPhoto = ({ control, userId }) => {
  ...
  const onChange = async (e) => {
    ...
    try {
      const id = Date.now();
      const photoRef = ref(storage, `photo/${userId}`);
  ...
  return (
    <div className="block rounded-full px-2">
      <label className="relative block overflow-hidden rounded-full border-2 border-gray-200 shadow">
