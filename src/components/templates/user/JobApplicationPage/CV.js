...
const CV = ({ control, user_id }) => {
  const { field, fieldState, formState } = useController({ control, name: 'cv', rules: { required: true } });
  const [uploadFile, uploading, snapshot, error] = useUploadFile();

  const onChange = async (e) => {
    if (e.target.files.length == 0) return;
    try {
      const fileRef = ref(storage, `cv/${user_id}-${e.target.files[0].name}`);
  ...
  return (
    <div className="flex mt-2">
      <label className="block w-32 mb-2 font-medium text-gray-900 dark:text-gray-300">CV</label>
      <div className="flex bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg w-64">
        <a
          href={field.value}
          target="_blank"
          rel="noreferrer"
